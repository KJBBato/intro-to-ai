...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in nxtvar.curDomain():
        nxtvar.setValue(val)
        constraintsOK = True
        for constr in csp.constraintsOf(nxtvar):
            if constr.numUnassigned() == 1:
                if FCCheck(constr, nxtvar, val) == "DWO":
                    constraintsOK = False
        if constraintsOK:
...
def GacEnforce(constraints, csp, reasonVar, reasonVal):
    ...
    while constraints:
        ...
        for var in constr.scope():
            for val in var.curDomain():
                if not constr.hasSupport(var, val):
                    var.pruneValue(val, reasonVar, reasonVal)
                    if var.curDomainSize() == 0:
                        return "DWO"
                    for v_constr in csp.constraintsOf(var):
                        if not v_constr in constraints and v_constr != constr:
                            constraints.append(v_constr)
    return "OK"
