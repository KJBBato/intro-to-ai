...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        scope = [qi, qj]
        iDomain = qi.curDomain()
        satList = []

        for r in range(len(iDomain)):

          jDomain = qj.curDomain()

          leftDiag = iDomain[r] - abs(i - j)
          rightDiag = iDomain[r] + abs(i - j)

          if iDomain[r] in jDomain:
            jDomain.remove(iDomain[r])
          if leftDiag in jDomain:
            jDomain.remove(leftDiag)
          if rightDiag in jDomain:
            jDomain.remove(rightDiag)

          for s in range(len(jDomain)):
            satList.append([iDomain[r], jDomain[s]])

        TableConstraint.__init__(self, self._name, scope, satList)
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        # Built upon AllDiffConstraint.check algorithm instead of equals we look for range
        ...
        for v in self.scope():
            if v.isAssigned():
                assignments.append(v.getValue())
                if v.getValue() == self.getRequiredVal():
                    counter += 1
            else:
                return True
        return (counter <= self.getUpperBound() and counter >= self.getLowerBound())
    ...
    def hasSupport(self, var, val):
        ...
        # Built upon AllDiffConstraint.hasSupport algorithm, same idea but we look for range

        if var not in self.scope():
            return True 

        def valsInRange(l):
            allVar = [var for (var, val) in l]
            allVal = [val for (var, val) in l]
            count = allVal.count(self.getRequiredVal())
            if count > self.getUpperBound():
                return False
            unAssignedCount = 0
            for var in self.scope():
                if var not in allVar:
                    varDom = var.curDomain()
                    if self.getRequiredVal() in varDom:
                        unAssignedCount += 1
            if count + unAssignedCount < self.getLowerBound():
                return False
            return True
