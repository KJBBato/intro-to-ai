...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        emissionModel[trueDistance] = Pr(noisyDistance | t rueDistance).
        ...
        * When a ghost is captured by Pacman, all beliefs should be
          ...
          prison cell with probability 1, this positions is
        ...
        #print 
        #print "---------------------------------------------------"
        #print "Probability at 2: ", emissionModel[2]
        #print 
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, p in newDist.items():
                allPossible[newPos] += self.beliefs[oldPos] * p
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(len(pos)):
            remainder = self.numParticles % len(pos)
            count = self.numParticles/len(pos)
            if i < remainder:
                count += 1
            self.particles.extend([pos[i] for j in range(count)])
            self.beliefs[pos[i]] = float(count)/float(self.numParticles)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            weights[self.getJailPosition()] = 1
        else:
            sample = []
            for i in range(self.numParticles):
                sample.append(util.sample(self.beliefs))
            for p in sample:
                manhattanDist = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[manhattanDist] > 0:
                    weights[p] = self.beliefs[p] * emissionModel[manhattanDist]
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        if weights.totalCount() == 0:
            ...
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(tuple(util.sample(weights)))
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            beliefs[particle] += float(1.0/self.numParticles)
        return beliefs
        "*** END YOUR CODE HERE ***"
